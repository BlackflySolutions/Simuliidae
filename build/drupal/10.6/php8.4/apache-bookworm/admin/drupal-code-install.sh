...
if [ -f /var/www/drupal/composer.json ] &&  [[ '--force' != $ARG1 ]]; then
  ...
  sudo -u drupal composer require --no-update drupal/core-recommended:${VSITE_DRUPAL_VER} --update-with-all-dependencies
  sudo -u drupal composer require --no-update drupal/core-composer-scaffold:${VSITE_DRUPAL_VER} --update-with-all-dependencies
  sudo -u drupal composer require --no-update drupal/core-project-message:${VSITE_DRUPAL_VER} --update-with-all-dependencies
